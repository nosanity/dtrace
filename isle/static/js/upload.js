...
$('body').delegate('.delete-material-btn', 'click', (e) => {
    ...
    let editor_id = $(e.target).parents('.summary-editor').find('textarea').attr('id')
    CKEDITOR.instances[editor_id].destroy();
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="result-helper-block">
                                ...
                                ${summary ?
                                `<span class="btn glyphicon glyphicon-pencil result-action-buttons pull-right edit-summary" data-file-id="${data['target_item_info']['material_id']}" data-type="${data['target_item_info']['type']}" title="Редактировать конспект"></span>`
                                :
                                `<span class="btn glyphicon glyphicon-pencil result-action-buttons pull-right edit-result-comment" title="Добавить/редактировать комментарий"></span>`
                                }
    ...
    if (pageType == 'loadMaterials_v2' || pageType == 'event_dtrace') {
        ...
        if (pageType == 'eventStructure') {
            ...
            if (isAssistant || summary) {
                edit_btn = `
                    <span value="${mId}" ${summary ? `data-is-summary="true" data-can-edit="true" data-type="event" data-file-id="${data['material_id']}"` : ``} class="btn glyphicon glyphicon-pencil result-action-buttons pull-right edit-event-block-material">
                    </span>`
            }
            html = `
