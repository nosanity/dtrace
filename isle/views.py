...
class EventsCsvData(IndexPageEventsFilterMixin, CSVResponseGeneratorMixin, View):
    ...
    def get(self, request):
        ...
        if CSVDump.current_generations_for_user(request.user) >= settings.MAX_PARALLEL_CSV_GENERATIONS:
            raise PermissionDenied
        csv_dump = CSVDump.objects.create(owner=request.user, header=obj.get_csv_filename(do_quote=False))
        meta_data['activity'] = self.activity_filter and self.activity_filter.id
        meta_data['context'] = request.user.chosen_context and request.user.chosen_context.id
        generate_events_csv.delay(csv_dump.id, [i.id for i in events], meta_data)
...
class CSVDumpsList(ListView):
    ...
    def get_queryset(self):
        return self.model.objects.filter(owner=self.request.user).order_by('-datetime_ordered')
